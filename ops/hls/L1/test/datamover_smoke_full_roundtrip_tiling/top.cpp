    ...
    const ops::hls::MemConfigTile& config)
{
    ...
    ops::hls::mem2streamTiled<MEM_DATA_WIDTH, BURST_SIZE, IN_ITR>(mem_in_b1, mem_in_b2, strm_data, config);

    // Write from stream to memory banks
    ops::hls::stream2memTiled<MEM_DATA_WIDTH, BURST_SIZE, IN_ITR>(strm_data, mem_out_b1, mem_out_b2, config);
}
...
#pragma HLS TOP
    ...
    unsigned short tile_size_x_n = tile_size_x;
    unsigned short overlap_size_x_n = overlap_size_x;
    unsigned short tile_size_y_n = tile_size_y;
    unsigned short overlap_size_y_n = overlap_size_y;
    ops::hls::genMemConfigTile<AXI_M_WIDTH, AXIS_WIDTH, 32>(gridSize, range, tile_size_x_n, overlap_size_x_n, tile_size_y_n, overlap_size_y_n, memconfig);
// #ifdef DEBUG_LOG
