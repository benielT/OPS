...
//DO NOT DELETE THIS AS THIS IS THE INITIAL VERSION OF THE FUNCTION

// /**
//  * @brief Transfers strided tile data from memory to a stream with configurable tiling and iteration parameters.
//  *
//  * This function reads a tiled region from memory and writes it to an output stream, handling
//  * multi-dimensional tile iteration with support for partial tiles at boundaries. The data is
//  * accessed with custom strides based on the tile configuration.
//  *
//  * @tparam MEM_DATA_WIDTH The bit-width of each memory access (e.g., 64, 128, 256)
//  * @tparam IN_ITR Number of input iterations for the underlying mem2stream operation (default: 2)
//  * @tparam BURST_SIZE Burst size for memory transfers (default: 32)
//  *
//  * @param mem_in Pointer to input memory containing the tile data
//  * @param strm_out Reference to the output HLS stream receiving the tile data
//  * @param config Reference to MemConfigTile configuration containing:
...
//  * @param stride_start Starting stride offset within the Y-dimension (default: 0)
//  *
//  * @note Uses HLS PIPELINE pragma on the innermost loop for hardware optimization
...
// /**
//  * @brief Merge/interleave two input HLS streams (two memory banks) into a single output stream
//  *        according to a linearized row parity within a tiled 3D layout.
//  *
//  * @tparam MEM_DATA_WIDTH  Bit-width of each element carried on the input/output streams (ap_uint<...>).
//  * @tparam IN_ITR          Pipeline initiation-interval hint used by the inner loop (controls II pragma).
//  *
//  * @param strm_in_b1       Input stream for bank 1.
//  * @param strm_in_b2       Input stream for bank 2.
//  * @param strm_out         Output stream receiving elements selected from bank 1 or bank 2.
//  * @param config           Tile layout and tiling metadata (start_z, end_z, tile sizes, last tile sizes,
//  *                         tile counts, etc.). Used to compute absolute (linearized) row indices.
//  *
//  * @details High-level behavior:
...
//  *
//  * @par Absolute-index computation (conceptual):
...
//  *   - realized_tile_y_z_span : (tile_size_y or last_tile_size_y for this x tile) * z_diff
//  *   - nominal_tile_y_z_span : nominal tile_size_y * z_diff (used when accumulating tile_y blocks)
//  *
//  * @par Skewed tiles and the z dimension not being tiled:
...
//  *
//  * @note The routine is intended for HLS synthesis; the inner-most loop is pipelined (II driven by
//  *       template parameter IN_ITR) to help meet throughput targets.
//  *
//  * @warning The input streams must contain exactly the expected number of elements for the tiled region
//  *          described by config; underflow or overflow on reads will block or be undefined in a hardware
//  *          context. config fields (tile sizes, last tile sizes, tile counts, start_z/end_z) must be
//  *          consistent and produce non-zero extents (e.g., end_z > start_z). Function relies on parity
//  *          of the computed linear index to select banks; ensure the intended bank-mapping semantics
//  *          match upstream/downstream memory layout.
//  */
...
//  *
//  * This function reads data from an input stream and writes it to memory according to a
...
//  *
//  * @tparam MEM_DATA_WIDTH The width of the memory data bus in bits
//  * @tparam IN_ITR Initiation interval for the pipeline. Default is 2.
//  * @tparam BURST_SIZE The size of burst writes to memory. Default is 32.
//  *
//  * @param[in] strm_in Input stream containing the tile data to be written
//  * @param[out] mem_out Pointer to the output memory where tile data will be written
//  * @param[in] config Memory configuration object containing tile dimensions, offsets,
//  *                    and grid information (tile counts, sizes, effective sizes, start/end positions)
//  * @param[in] stride_start Starting offset for the Y dimension iteration. Default is 0.
//  *                         Allows partial tile processing starting from a specific Y position.
//  *
//  * @note This function is designed for HLS synthesis and includes pipeline pragmas
//  *       for performance optimization with an initiation interval of IN_ITR.
//  * @note Debug logging can be enabled by defining DEBUG_LOG macro for detailed
//  *       offset and iteration information.
//  * @note The function processes tiles in nested loops over Y, X, Z dimensions with
//  *       stride of 2 in the Y dimension for each pipeline iteration.
//  */
// template <unsigned short MEM_DATA_WIDTH, unsigned short IN_ITR=2, unsigned short BURST_SIZE=32>
...
#pragma HLS TOP
    ...
    ops::hls::genMemConfigTileV2<AXI_M_WIDTH, 32>(gridSize, range, tileSize, tileCount, overlapSize, effectiveTileSize, lastTileSize, total_xblocks, memconfig);   
