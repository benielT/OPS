...
class FInteger(FPrimitive):
    ...
    def asC(self) -> str:
        if self.kind == 4:
            ...
            raise OpsError(f"FInteger unknown kind: {self.kind}")
...
class FReal(FPrimitive):
    ...
    def asC(self) -> str:
        if self.kind == 4:
            ...
            raise OpsError(f"FReal unknown kind: {self.kind}")
...
def parseConstsModule(program: Program, info: Info) -> Dict[str, FType]:
    ...
    if module is None:
        raise OpsError("Invalid consts module")
...
def parseSubroutineInfo(entity_ast) -> SubprogramInfo:
    subroutine_stmt = fpu.get_child(entity_ast, f2003.Subroutine_Stmt)
    spec_part = fpu.get_child(entity_ast, f2003.Specification_Part)

    name = translateName(fpu.get_child(subroutine_stmt, f2003.Name))
    sub_info = SubprogramInfo(name, entity_ast)
...
def parseIntrinsicType(intrinsic_type_spec: f2003.Intrinsic_Type_Spec, ctx: Context) -> FType:
    ...
    if intrinsic_type_spec.items[1] is not None:
        if isinstance(intrinsic_type_spec.items[1], f2003.Kind_Selector):
            kind = str(intrinsic_type_spec.items[1])

    if type_name == "INTEGER":
        if kind in [None, "(4)", "(kind=4)", "(kind = 4)", "(KIND=4)", "(KIND = 4)"]:
            return FInteger(4)
        elif kind in ["(8)", "(kind=8)", "(kind = 8)", "(KIND=8)", "(KIND = 8)"]:
            return FInteger(8)

    elif type_name == "REAL":
        if kind in ["(4)", "(kind=4)", "(kind = 4)", "(KIND=4)", "(KIND = 4)"]:
            return FReal(4)
        elif kind in [None, "(8)", "(kind=8)", "(kind = 8)", "(KIND=8)", "(KIND = 8)"]:
            return FReal(8)
...
def setOpArg(entity: Entity, param_idx: int, info: Info, ops_arg: OPS.Arg) -> None:
    ...
    sub_info.params[param_idx].ops_arg = ops_arg


def resolveOpArgs(entities: List[Entity], info: Info, loop: OPS.Loop) -> None:
        ...
        if isinstance(node, f2003.Implicit_Part):
            for implicit_node in node.children:
                ...
                if isinstance(implicit_node, f2003.Implicit_Stmt) and implicit_node.items[0] == "NONE":
                    ...
                    ctx.error(f"Unsupported implicit part statement", implicit_node)
...
#        ctx.error(f"No implicit none", spec_part)
    ...
    for name, type_ in ctx.sub_info.types.items():
        if name in ctx.sub_info.paramNames():
            if isinstance(type_, FArray):
                sizes = type_.spanSizes()
                c_type = type_.inner.asC()
                kernel_param[name] = [c_type, sizes]
            if isinstance(type_, FPrimitive):
                c_type = type_.asC()
                kernel_param[name] = [c_type, ['0']]
...
def translateComment(comment: f2003.Comment, ctx: Context) -> str:
    return comment.items[0] 
...
TRANSLATE_TABLE = {
    ...
    # Comment
    f2003.Comment: translateComment,
