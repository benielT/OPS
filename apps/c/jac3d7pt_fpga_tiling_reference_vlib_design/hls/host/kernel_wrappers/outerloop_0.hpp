// Auto-generated at 2026-01-27 16:58:13.317544 by ops-translator
#pragma once 
...
public:
            ...
            m_datamoverName("datamover_outerloop_0")
    {
        ...
        OCL_CHECK(err, m_stream_step_down = cl::Kernel(m_fpga->getProgram(), "stream_step_down", &err));
        OCL_CHECK(err, m_stream_step_up = cl::Kernel(m_fpga->getProgram(), "stream_step_up", &err));
    }
    ...
    )
    {
        ...
        ops::hls::SizeType2d overlap_size = {get_overlap_size<2, 2, 1, mem_vector_factor>(), 
                get_overlap_size<2, 2, 1, 1>()};
		...
		unsigned int total_iter_par_factor = iter_par_factor * 2;
...
#endif 
        narg = 0;
        OCL_CHECK(err, err = m_stream_step_down.setArg(narg++, total_xblocks_widen));
        OCL_CHECK(err, err = m_stream_step_down.setArg(narg++, total_xblocks_widen * 2));
        OCL_CHECK(err, err = m_stream_step_down.setArg(narg++, adjusted_outer_iter));

        narg = 0;
        OCL_CHECK(err, err = m_stream_step_up.setArg(narg++, total_xblocks_widen));
        OCL_CHECK(err, err = m_stream_step_up.setArg(narg++, total_xblocks_widen * 2));
        OCL_CHECK(err, err = m_stream_step_up.setArg(narg++, adjusted_outer_iter));
        ...
        OCL_CHECK(err, err = m_fpga->getCommandQueue().enqueueTask(m_stream_step_up, &activeEvents, &event_s_up));
        ...
        OCL_CHECK(err, err = m_fpga->getCommandQueue().enqueueTask(m_stream_step_down, &activeEvents, &event_s_down));
    ...
    execEvents.push_back(event_s_down);
    execEvents.push_back(event_s_up);
    ...
    registerProfileEvents();
        ...
        arg0.activeEvents.push_back(event_s_down);
        arg0.activeEvents.push_back(event_s_up);
        ...
        arg1.activeEvents.push_back(event_s_down);
        arg1.activeEvents.push_back(event_s_up);
...
private:
    ...
    cl::Kernel m_stream_step_down;
    cl::Kernel m_stream_step_up;
