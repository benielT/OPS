// Auto-generated at 2025-09-06 21:18:49.928430 by ops-translator
...

{
    ...
    const unsigned int bytes_per_pkt = sizeof(stencil_type) * vector_factor;
    const unsigned int total_bytes = num_pkts * bytes_per_pkt;

    printf("[KERNEL_DEBUG]|%s| num_pkts=%u, bytes_per_pkt=%u, total_bytes=%u\n", __func__, num_pkts, bytes_per_pkt, total_bytes);

// #ifdef DEBUG_LOG
    #ifndef OPS_TILING
        printf("[KERNEL_DEBUG]|%s| stencilConfig: dim=%hu, grid_size=[%hu,%hu,%hu], lower_limit=[%hu,%hu,%hu], upper_limit=[%hu,%hu,%hu], total_itr=%u, outer_loop_limit=%hu, batch_size=%hu\n", 
            __func__, stencilConfig.dim, 
            ...
            stencilConfig.total_itr, stencilConfig.outer_loop_limit, stencilConfig.batch_size);
    #else
        printf("[KERNEL_DEBUG]|%s| stencilConfig: dim=%hu, grid_size=[%hu,%hu,%hu], outer_loop_limit=%hu, tile_size=[%hu,%hu], last_tile_size=[%hu,%hu], tile_count=[%hu,%hu]\n", 
            __func__, stencilConfig.dim, 
            stencilConfig.grid_size[0], stencilConfig.grid_size[1], stencilConfig.grid_size[2],
